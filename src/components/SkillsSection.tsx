    ...
    icon: (
      <svg viewBox="0 0 128 128" className="w-10 h-10">
        ...
        <linearGradient id="flutter-a" gradientUnits="userSpaceOnUse" x1="59.365" y1="116.36" x2="86.825" y2="99.399">
          <stop offset="0" stopColor="#1b4e94"/><stop offset=".63" stopColor="#1a5497"/><stop offset="1" stopColor="#195a9b"/>
        </linearGradient>
...
const categoryInfo = [
  { name: "Frontend", icon: Layers, description: "Modern web interfaces" },
  { name: "Backend", icon: Server, description: "Server-side development" },
  { name: "Mobile", icon: Code2, description: "Cross-platform apps" },
  { name: "Systems", icon: Code2, description: "Low-level programming" },
];
      ...
      <div ref={ref} className="max-w-5xl mx-auto fade-in-up relative z-10">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-6 mb-12">
          {technologies.map((tech, index) => (
            <div
              key={tech.name}
              className="group relative flex flex-col items-center p-4 rounded-2xl border border-border bg-card/80 backdrop-blur-sm
                neon-card-hover holo-shimmer transition-all duration-500"
        ...
        <div className="grid md:grid-cols-4 gap-4">
          {categoryInfo.map((category, index) => {
            ...
            return (
              <div
                key={category.name}
                className="group relative flex items-center gap-3 p-4 rounded-2xl border border-border bg-card/80 backdrop-blur-sm
                  neon-card-hover holo-shimmer transition-all duration-500"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:bg-primary/20">
        ...
        <div className="mt-12 text-center">
          <div className="inline-flex items-center gap-4 px-6 py-4 rounded-2xl border border-primary/20 bg-primary/5 backdrop-blur-sm">
            <Code2 className="w-6 h-6 text-primary glow-text-cyan" />
